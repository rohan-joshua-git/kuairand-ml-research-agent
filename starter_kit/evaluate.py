"""
KuaiRand-Pure official evaluation script -- authoritative, do not modify.
...
Ranking scope: each user is ranked only within their own rows in the eval split,
               not against a global item pool.
Zero-positive users: nDCG recorded as 0.0 and included in the mean (consistent with CWM).
              GAUC only counts users with 0 < positives < impressions, weighted by positive count.
...
Vendored verbatim from the organizer-provided KuaiRand-Pure Starter Kit.
"""
...
def ndcg_at_k(labels, k):
    """labels are already sorted by predicted score, descending."""
