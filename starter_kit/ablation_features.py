"""Wire in CWM's 13 feature columns; check whether user-side features help under the FM baseline.

Reproducible experiment. Result (organizer-run): adding features bought ~0.
...
def build(mode):
    """mode: 'base'=5 fields (current kit) / 'item'=item-side features added / 'cwm13'=all 13 CWM fields"""
...
for mode,desc in [('base','5 fields (current kit)'),('item','+4 item-side = 9 fields'),('cwm13','all 13 CWM fields')]:
    ...
    print(f"{desc:20s} ({nf:2d}f) | test GAUC {g:.4f} | nDCG@5 {n5:.4f} | primary {pr:.4f} +/- {sd:.4f}")
