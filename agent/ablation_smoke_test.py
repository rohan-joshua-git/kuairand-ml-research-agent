...
def main() -> None:
    ...
    # run_ablation runs its probes as fresh subprocesses (see agent/ablation.py),
    # so it takes no in-memory split — it always measures the code on disk.
    results = run_ablation(baseline_metrics, variants=variants)
    if not results:
